...
def nba_ranking(data, column, by, top=10, ascending=True, fun='mean'):
    ...
    if top > number_elements:
        # warnings.warn(f'The number of elements in the ranking
        # is smaller than the one provided. \n Changing top value
        # from {top} to {number_elements}', Warning)
        top = number_elements
    ...
    transformed_data = data[[column, by]].groupby(column).agg(
        fun).sort_values(by=by, ascending=ascending).head(top).reset_index()
